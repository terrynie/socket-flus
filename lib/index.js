...
module.exports = class SocketFlus {
  constructor(options) {
    ...
    this.sockets = [];
    process.env.TEST_INTERVAL = options.duration ? Math.floor(options.duration * 1000 / this.requestCount) : 0;
    return this;
  }
  ...
  connTest() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      let endAt = null;
      let errCount = 0;
      connect(that.url.pop(), that.connectCount)
  ...
  messageTest(event, message) {
    ...
    return new Promise(async (resolve, reject) => {
      if(that.sockets.length < that.connectCount) {
        let conn = await connect(that.url.pop(), that.connectCount);
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.mode === Mode.REQUESTBYCOUNT) {
      ...
      for (let i = 0; i < options.count; i++) {
        ...
        util.sleep(process.env.TEST_INTERVAL);
      }
      result.finish = Date.now();
      resolve(result);
    }
